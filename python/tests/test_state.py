...
class AttentionStateDescriptorTest(unittest.TestCase):
            ...
            validate_descriptor(
                AttentionStateDescriptor(
                    ...
                    layout=LayoutMetadata(
                        layout="contiguous", page_tokens=1, block_tokens=1, packed=False
                    ),
                    compatibility_flags=[CompatibilityFlag.EXACT_REUSE],
                    transfer_paths=[
                        TransferPath(
                            backend=TransferBackend.BASELINE_TRANSPORT,
                            capabilities=[TransferCapability.HOST_TO_STORE],
                        )
                    ],
    ...
    def test_descriptor_supports_partial_materialization_for_paged_layouts(self) -> None:
        descriptor = AttentionStateDescriptor(
            ...
            transfer_paths=[
                TransferPath(
                    backend=TransferBackend.STAGED_COPY, capabilities=[TransferCapability.ASYNC]
                )
            ],
            materialization=MaterializationProfile(
                capabilities=[MaterializationCapability.PARTIAL],
                tier_kinds=[],
                device_classes=[],
                buffer_kinds=[],
            ),
