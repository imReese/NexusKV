...
def validate_descriptor(descriptor: AttentionStateDescriptor) -> None:
    if descriptor.schema_version != SCHEMA_VERSION:
        raise DescriptorValidationError(f"unsupported schema version: {descriptor.schema_version}")
...
def create_mla_descriptor(
    descriptor_id: str, engine_family: EngineFamily = EngineFamily.SGLANG
) -> AttentionStateDescriptor:
    ...
    return AttentionStateDescriptor(
        ...
        tensor_specs=[
            TensorSpec(
                ...
                dtype="bfloat16",
                shape=["num_pages", "page_size", "512"],
            ),
            TensorSpec(
                name="k_pe",
                role=TensorRole.POSITION,
                dtype="bfloat16",
                shape=["num_pages", "page_size", "64"],
            ),
        ],
    ...
    return AttentionStateDescriptor(
        ...
        tensor_specs=[
            ...
            TensorSpec(
                ...
                dtype="int32",
                shape=["num_pages", "top_k"],
            ),
        ],
        quantization=QuantizationMetadata(scheme="none", bits=16, group_size=0),
        layout=LayoutMetadata(
            layout="sparse_indexed", page_tokens=16, block_tokens=16, packed=False
        ),
        compatibility_flags=[CompatibilityFlag.EXACT_REUSE, CompatibilityFlag.PAGE_REUSE],
        transfer_paths=[
            TransferPath(
...
def create_kda_descriptor(
    descriptor_id: str, engine_family: EngineFamily = EngineFamily.VLLM
) -> AttentionStateDescriptor:
    ...
    return AttentionStateDescriptor(
        ...
        layout=LayoutMetadata(
            layout="recurrent_checkpoint", page_tokens=0, block_tokens=0, packed=True
        ),
        ...
        layout_metadata={
            ...
            "checkpoint_boundary": "terminal_recurrent_state",
        },
    )


def create_csa_descriptor(
    descriptor_id: str, engine_family: EngineFamily = EngineFamily.VLLM
) -> AttentionStateDescriptor:
    ...
    return AttentionStateDescriptor(
        ...
        tensor_specs=[
            TensorSpec(
                name="csa_compressed_kv",
                role=TensorRole.LATENT,
                dtype="fp4",
                shape=["num_layers", "blocks", "dim"],
            ),
            TensorSpec(
                name="csa_topk_indices",
                role=TensorRole.AUXILIARY,
                dtype="int32",
                shape=["num_layers", "topk"],
            ),
        ],
        quantization=QuantizationMetadata(scheme="fp4", bits=4, group_size=4),
        layout=LayoutMetadata(
            layout="csa_4token_sparse", page_tokens=0, block_tokens=4, packed=True
        ),
        compatibility_flags=[
            CompatibilityFlag.EXACT_REUSE,
            CompatibilityFlag.PREFIX_REUSE,
            CompatibilityFlag.BLOCK_REUSE,
        ],
        transfer_paths=[
            TransferPath(
...
def create_hca_descriptor(
    ...
    return AttentionStateDescriptor(
        ...
        tensor_specs=[
            TensorSpec(
                name="hca_summary_vec",
                role=TensorRole.LATENT,
                dtype="float16",
                shape=["num_layers", "summary_dim"],
            ),
        ],
        quantization=QuantizationMetadata(scheme="none", bits=16, group_size=0),
        layout=LayoutMetadata(
            layout="hca_128token_summary", page_tokens=0, block_tokens=128, packed=True
        ),
        compatibility_flags=[
            CompatibilityFlag.EXACT_REUSE,
            CompatibilityFlag.PREFIX_REUSE,
            CompatibilityFlag.WARM_START,
        ],
        transfer_paths=[
            TransferPath(backend=TransferBackend.ZERO_COPY, capabilities=[TransferCapability.ASYNC])
        ],
        materialization=MaterializationProfile(
            capabilities=[
                MaterializationCapability.FULL,
    ...
    return AttentionStateDescriptor(
        ...
        tensor_specs=[
            TensorSpec(
                ...
                role=TensorRole.KEY,
                dtype="float16",
                shape=["num_shards", "page_tokens", "dim"],
            ),
        ],
        quantization=QuantizationMetadata(scheme="none", bits=16, group_size=0),
        layout=LayoutMetadata(
            layout="dspark_sharded_page", page_tokens=16, block_tokens=16, packed=False
        ),
        compatibility_flags=[CompatibilityFlag.PAGE_REUSE, CompatibilityFlag.PREFIX_REUSE],
        transfer_paths=[
            TransferPath(backend=TransferBackend.RDMA, capabilities=[TransferCapability.ASYNC])
        ],
        materialization=MaterializationProfile(
            capabilities=[
                MaterializationCapability.FULL,
    ...
    return AttentionStateDescriptor(
        ...
        tensor_specs=[
            TensorSpec(
                ...
                role=TensorRole.LATENT,
                dtype="bfloat16",
                shape=["num_layers", "state_dim"],
            ),
        ],
        quantization=QuantizationMetadata(scheme="none", bits=16, group_size=0),
        layout=LayoutMetadata(
            layout="gdn_recurrent_state", page_tokens=0, block_tokens=1, packed=True
        ),
        compatibility_flags=[CompatibilityFlag.EXACT_REUSE, CompatibilityFlag.WARM_START],
        transfer_paths=[
            TransferPath(backend=TransferBackend.ZERO_COPY, capabilities=[TransferCapability.ASYNC])
    ...
    return AttentionStateDescriptor(
        ...
        tensor_specs=[
            TensorSpec(
                ...
                role=TensorRole.LATENT,
                dtype="bfloat16",
                shape=["num_layers", "d_state", "d_inner"],
            ),
        ],
        quantization=QuantizationMetadata(scheme="none", bits=16, group_size=0),
